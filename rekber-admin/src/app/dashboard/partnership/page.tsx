...
export default function PartnershipPage() {
  ...
  if (loading && users.length === 0) {
    return (
      <div className="min-h-screen bg-muted/50 flex flex-col items-center justify-center gap-3">
        ...
        <p className="text-muted-foreground text-sm font-medium">Memuat data partner...</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4 font-sans text-foreground">
      ...
      {/* HEADER UTAMA */}
      <div>
        <h1 className="text-2xl sm:text-3xl font-bold text-balance">Manajemen Partner</h1>
        <p className="text-sm sm:text-base text-muted-foreground text-balance">Kelola semua mitra partnership aktif di platform.</p>
      </div>
      ...
      <Card className="bg-white rounded-sm overflow-hidden">
        <div className="px-4 py-3 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2 shrink-0">
            ...
            <h3 className="text-sm font-semibold text-foreground">Pencarian Partner</h3>
          </div>
        ...
        <CardHeader className="px-4 py-3 border-b border-muted bg-white flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
          <div className="space-y-1">
            <div className="flex items-center gap-2">
              <CardTitle className="text-base font-semibold text-foreground">Daftar Partner</CardTitle>
              {/* INDIKATOR TOTAL DATA */}
              <Badge variant="secondary" className="bg-purple-50 text-purple-700 text-[10px] font-medium px-2 py-0.5 border border-purple-200">
                 {pagination?.total_items || 0} Total Partner
              </Badge>
            </div>
                ...
                {users.length > 0 ? (
                  users.map((user) => (
                    <TableRow key={user.id} className="border-muted hover:bg-muted/50 transition-colors">
                      ...
                      <TableCell className="py-2">
                        {user.is_partnership ? (
                          <Badge className="bg-purple-100 text-purple-700 hover:bg-purple-100 border border-purple-200 font-semibold text-[10px] rounded px-1.5 py-0.5 shadow-none flex w-fit gap-1 items-center">
                            Active ({user.partnership_percentage}%)
                          </Badge>
                        ) : (
                  ...
                  <TableRow>
                    <TableCell colSpan={7} className="text-center py-12">
                      <div className="flex flex-col items-center justify-center gap-2">
                        ...
                        <p className="text-muted-foreground text-xs font-medium">Tidak ada data partner ditemukan.</p>
                        {hasActiveFilters && (
